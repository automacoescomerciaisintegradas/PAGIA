...
const DEFAULT_CONFIG: PAGIACLIConfig = {
    version: '1.0.0',
    base_install: true,
...
class CLIConfigReader {
    private config: PAGIACLIConfig | null = null;
    ...
    load(): PAGIACLIConfig {
        ...
        try {
            ...
            const parsed = parseYaml(content) as Partial<PAGIACLIConfig>;
    ...
    getDefaultProvider(): string {
        ...
        return profile.ai_provider || this.get<string>('ai_provider.default') || 'groq';
    }
    ...
    getEnabledAgents(): string[] {
        const profile = this.getActiveProfile();
        return profile.agents || this.get<string[]>('agents.enabled') || ['analyst', 'coder'];
    }
    ...
    getExecutionMode(): 'interactive' | 'autonomous' {
        const profile = this.getActiveProfile();
        return profile.execution_mode || this.get<'interactive' | 'autonomous'>('agents.execution_mode') || 'interactive';
    }
    ...
    isEnabled(feature: 'mcp' | 'tdd' | 'llm_gateway' | 'claude_code_commands' | 'pagia_commands'): boolean {
        switch (feature) {
            case 'mcp':
                return this.get<boolean>('mcp.enabled') ?? true;
            case 'tdd':
                return this.get<boolean>('tdd.enabled') ?? true;
            case 'llm_gateway':
                return this.get<boolean>('llm_gateway.enabled') ?? true;
            case 'claude_code_commands':
                return this.get<boolean>('claude_code_commands') ?? true;
            case 'pagia_commands':
                return this.get<boolean>('pagia_commands') ?? true;
            default:
                return false;
        }
    }
     ...
     */
    getLLMGatewayConfig(): { port: number; localUrl: string; providers: string[] } {
        return {
            port: this.get<number>('llm_gateway.port') || 3000,
            localUrl: this.get<string>('llm_gateway.local_llm_url') || 'http://localhost:8080',
            providers: this.get<string[]>('llm_gateway.providers') || ['local'],
        };
    }
     ...
     * Get MCP config
     */
    getMCPConfig(): { port: number; tools: string[] } {
        return {
            port: this.get<number>('mcp.port') || 3100,
            tools: this.get<string[]>('mcp.tools') || [],
        };
    }
     ...
     */
    getInterfaceConfig(): { theme: string; language: string; showBanner: boolean; showTips: boolean; debug: boolean } {
        return {
            theme: this.get<string>('interface.theme') || 'auto',
            language: this.get<string>('interface.language') || 'pt-BR',
            showBanner: this.get<boolean>('interface.show_banner') ?? true,
            showTips: this.get<boolean>('interface.show_tips') ?? true,
            debug: this.get<boolean>('interface.debug') ?? false,
        };
    }
     ...
     */
    private mergeWithDefaults(parsed: Partial<PAGIACLIConfig>): PAGIACLIConfig {
        return {
            ...DEFAULT_CONFIG,
            ...parsed,
...
let instance: CLIConfigReader | null = null;

export function getCLIConfig(projectRoot?: string): CLIConfigReader {
    if (!instance) {
