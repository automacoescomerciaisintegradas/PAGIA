...
function getRouterConfigPath(): string {
    ...
    if (isInPagiaProject()) {
        ...
        if (existsSync(rootPagiaRouter)) return rootPagiaRouter;
    }
...
export class RouterManager {
    ...
    private config: RouterSystemConfig | null = null;
    ...
    private async createDefaultConfig(): Promise<void> {
        ...
        const config: RouterSystemConfig = {
            version: '1.0.0',
            providers,
            router: {
     ...
     */
    private createProviderConfig(providerName: string, apiKey: string): ProviderConfig | null {
        const providerConfigs: Record<string, Omit<ProviderConfig, 'api_key'>> = {
            gemini: {
                name: 'gemini',
    ...
    loadConfig(): RouterSystemConfig {
        ...
        try {
            ...
            return JSON.parse(content) as RouterSystemConfig;
        } catch {
            return {
                version: '1.0.0',
                providers: [],
     ...
     */
    async saveConfig(config: RouterSystemConfig): Promise<void> {
        const configPath = getRouterConfigPath();
     ...
     */
    async addModel(providerName: string, model: string): Promise<boolean> {
        const config = this.getConfig();
     ...
     */
    async removeModel(providerName: string, model: string): Promise<boolean> {
        const config = this.getConfig();
        const provider = config.providers.find(p => p.name === providerName);
    ...
    async setRouter(
        ...
        if (type === 'longContextThreshold') {
            (config.router as any)[type] = parseInt(model);
        } else {
            (config.router as any)[type] = { provider, model };
        }
    ...
    async exportPreset(
        ...
        // Sanitize API keys
        const sanitizedConfig: RouterSystemConfig = {
            ...config,
     ...
     */
    getActivationScript(shell: 'bash' | 'powershell' | 'cmd' = 'bash'): string {
        const config = this.getConfig();
