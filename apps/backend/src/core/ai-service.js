...
export class AIService {
     ...
     */
    async chatWithFallback(messages, fallbackIndex = 0) {
     ...
     */
    async chatInternal(messages, modelOverride) {
     ...
     */
    async chatGeminiREST(messages, model) {
     ...
     */
    async chatOpenAICompatible(messages) {
 ...
 */
function getApiKeyForProvider(type, configApiKey) {
    if (configApiKey)
        return configApiKey;
    const envMapping = {
 ...
 */
function getDefaultModelForProvider(type, configModel) {
    if (configModel)
        return configModel;
    const defaults = {
...
export function createAIService(config) {
    ...
    let type = (config?.type || process.env.AI_PROVIDER);
    ...
    if (!type && routerConfig && routerConfig.router?.default) {
        type = routerConfig.router.default.provider;
