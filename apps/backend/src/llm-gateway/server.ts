...
import { UnifiedChatRequest, UnifiedChatResponse } from './types.js';
...
export class LLMGatewayServer {
    ...
    private async handleChatCompletion(req: express.Request, res: express.Response) {
        try {
            ...
            if (req.headers['x-goog-api-key']) headers['x-goog-api-key'] = req.headers['x-goog-api-key']; // Gemini
            ...
            if (unifiedRequest.stream) {
                ...
                upstreamResponse.body.on('data', (chunkBuffer: Buffer) => {
                ...
                upstreamResponse.body.on('error', (err: any) => {
