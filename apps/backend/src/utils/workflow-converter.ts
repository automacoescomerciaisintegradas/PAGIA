...
export function detectFormat(json: any): 'n8n' | 'node-red' | 'unknown' {
...
export function convertN8nToPagia(name: string, n8nData: any): string {
    const nodes: PagiaNode[] = n8nData.nodes.map((node: any) => {
        let agentId = 'analyst';
        const type = node.type.toLowerCase();

        if (type.includes('code') || type.includes('function')) agentId = 'developer';
        if (type.includes('webhook') || type.includes('trigger')) agentId = 'planner';
        if (type.includes('if') || type.includes('switch') || type.includes('filter')) agentId = 'analyst';
...
export function convertNodeRedToPagia(name: string, nodeRedNodes: any[]): string {
    ...
    const nodes: PagiaNode[] = flowNodes.map(node => {
        let agentId = 'analyst';
        const type = node.type.toLowerCase();

        if (type === 'function' || type === 'exec') agentId = 'developer';
        if (type === 'inject') agentId = 'planner';
        if (type === 'debug') agentId = 'analyst';
        if (type.includes('switch') || type.includes('change')) agentId = 'analyst';
        if (type.includes('request')) agentId = 'developer'; // HTTP requests usually
    ...
    flowNodes.forEach(node => {
        if (node.wires && Array.isArray(node.wires)) {
            ...
            node.wires.forEach((outputConnections: string[], outputIndex: number) => {
...
function generateYaml(name: string, nodes: PagiaNode[], edges: PagiaEdge[], sourceDescription: string): string {
    ...
    const nodesYaml = nodes.map(n =>
        `  - id: "${n.id}"\n    name: "${n.name}"\n    agentId: ${n.agentId}\n    description: "${n.description || ''}"`
    ).join('\n\n');

    const edgesYaml = finalEdges.map(e =>
        `  - from: "${e.from}"\n    to: "${e.to}"`
    ).join('\n');
