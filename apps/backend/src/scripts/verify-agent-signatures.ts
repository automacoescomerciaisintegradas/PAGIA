...
interface AgentReport {
    ...
    path: string;
    signature: string;
    status: 'CORRECT' | 'OLD' | 'GENERIC' | 'CUSTOM' | 'NONE';
    size: number;
}

interface VerificationReport {
    totalAgents: number;
    correctSignatures: number;
    oldSignatures: number;
    genericSignatures: number;
    customSignatures: number;
    noSignature: number;
    agentsByStatus: Record<string, AgentReport[]>;
    directoriesChecked: string[];
}

function detectSignature(content: string): { signature: string; type: string } {
...
function verifyDirectory(dirPath: string): AgentReport[] {
...
function generateReport(reports: AgentReport[]): VerificationReport {
