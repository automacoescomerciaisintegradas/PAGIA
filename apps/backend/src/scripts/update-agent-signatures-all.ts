...
for (const dir of AGENTS_DIRS) {
    try {
                ...
                try {
                    ...
                    if (content.includes(OLD_SIGNATURE)) {
                        ...
                        const newContent = content.replace(
                            new RegExp(OLD_SIGNATURE.replace(/[.*+?^${}()|[\\]/g, '\$&'), 'g'),
                            NEW_SIGNATURE
                        );
        ...
        console.error(`✗ Erro ao acessar diretório ${dir}:`, (error as Error).message);
    }
}
...
try {
    ...
    console.error('Erro ao processar arquivos na raiz:', (error as Error).message);
}
