...
function getProviderConfigs(): Record<ProviderName, ProviderConfig> {
    const baseConfigs: Record<ProviderName, ProviderConfig> = {
        openai: {
            name: 'openai',
    ...
    try {
        ...
        if (rConfig && rConfig.providers) {
            for (const p of rConfig.providers) {
                if (baseConfigs[p.name as ProviderName]) {
                    const bc = baseConfigs[p.name as ProviderName];
...
export class MultiProvider implements AIProvider {
    ...
    constructor(providerName?: ProviderName) {
        ...
        let selectedProvider = (providerName || process.env.AI_PROVIDER) as ProviderName;

        // If no explicit provider, try the router's default
        if (!selectedProvider && rConfig?.router?.default) {
            selectedProvider = rConfig.router.default.provider as ProviderName;
        }
    ...
    private async callOpenAICompatible(prompt: string, options?: AIProviderOptions): Promise<string> {
        const messages: any[] = [];
    ...
    private async callGemini(prompt: string, options?: AIProviderOptions): Promise<string> {
    ...
    private async callOllama(prompt: string, options?: AIProviderOptions): Promise<string> {
    ...
    static listAvailableProviders(): { name: string; configured: boolean; model: string }[] {
        const configs = getProviderConfigs();
