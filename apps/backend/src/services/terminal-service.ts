...
// Tema PAGIA Premium
const PAGIA_THEME: TerminalTheme = {
    name: 'PAGIA Premium',
    background: '#0d1117',
    foreground: '#c9d1d9',
    cursor: '#58a6ff',
...
const PAGIA_KEYBINDINGS: KeyBinding[] = [
...
export class TerminalService {
    ...
    private supportedTerminals: Map<string, () => Promise<TerminalInfo>>;
     ...
     */
    async detectAll(): Promise<TerminalInfo[]> {
     ...
     */
    async detectCurrent(): Promise<TerminalInfo | null> {
     ...
     */
    async configure(terminal: string, config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
        const configFunctions: Record<string, (config: Partial<TerminalConfig>) => Promise<{ success: boolean; message: string; configPath?: string }>> = {
     ...
     */
    private normalizeTerminalName(name: string): string {
     ...
     */
    async applyKeybindings(terminal: string, keybindings?: KeyBinding[]): Promise<{ success: boolean; message: string }> {
     ...
     */
    async applyTheme(terminal: string, theme?: TerminalTheme): Promise<{ success: boolean; message: string }> {
    ...
    private async detectKitty(): Promise<TerminalInfo> {
    ...
    private async detectAlacritty(): Promise<TerminalInfo> {
    ...
    private async detectGhostty(): Promise<TerminalInfo> {
        let configPath: string;
        if (this.os === 'darwin') {
    ...
    private async detectWarp(): Promise<TerminalInfo> {
    ...
    private async detectZed(): Promise<TerminalInfo> {
        const configPath = this.os === 'darwin'
    ...
    private async detectWindowsTerminal(): Promise<TerminalInfo> {
    ...
    private async detectITerm2(): Promise<TerminalInfo> {
    ...
    private async detectHyper(): Promise<TerminalInfo> {
    ...
    private async detectVSCode(): Promise<TerminalInfo> {
        let configPath: string;
        if (this.os === 'win32') {
    ...
    private async configureKitty(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureAlacritty(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
        let configPath: string;
        if (this.os === 'win32') {
        ...
        if (config.keybindings) {
            ...
            for (const kb of config.keybindings) {
                ...
                if (alacrittyKey.mods) content += `mods = "${alacrittyKey.mods}"\n`;
    ...
    private async configureGhostty(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
        let configPath: string;
        if (this.os === 'darwin') {
    ...
    private async configureWarp(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureZed(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureWindowsTerminal(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureITerm2(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureHyper(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
    ...
    private async configureVSCode(config: Partial<TerminalConfig>): Promise<{ success: boolean; message: string; configPath?: string }> {
        ...
        if (config.theme) {
            (vscodeSettings['workbench.colorCustomizations'] as Record<string, string>) = {
                'terminal.background': config.theme.background,
                'terminal.foreground': config.theme.foreground,
    ...
    private convertKeyToKitty(key: string): string {
        return key
            .replace('ctrl+', 'ctrl+')
            .replace('shift+', 'shift+')
            .replace('alt+', 'alt+')
    ...
    private convertKeyToAlacritty(key: string): { key: string; mods?: string } {
    ...
    private convertKeyToGhostty(key: string): string {
        return key
            .replace('ctrl+', 'ctrl+')
            .replace('shift+', 'shift+')
            .replace('alt+', 'alt+');
