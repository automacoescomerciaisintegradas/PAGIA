...
const PROVIDERS: { value: CredentialProvider; name: string; envVar: string }[] = [
...
authCommand
    ...
    .action(async (provider: string | undefined, options: { key?: string; baseUrl?: string; model?: string }) => {
        const credentials = getCredentialsManager();

        try {
            let selectedProvider: CredentialProvider;
            let apiKey: string;
...
authCommand
    ...
    .action(async (provider: string | undefined, options: { all?: boolean }) => {
        ...
        try {
            ...
            const deleted = await credentials.delete(provider as CredentialProvider);
...
authCommand
    ...
    .action(async (provider: string | undefined) => {
        ...
        try {
            ...
            const hasCredential = await credentials.has(provider as CredentialProvider);
            if (!hasCredential) {
                console.log(chalk.yellow(`Nenhuma credencial encontrada para ${provider}.`));
