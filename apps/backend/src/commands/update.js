...
updateCommand
    ...
    try {
        ...
        if (options.ai) {
            ...
            const response = await aiService.generate(`Analise estas tarefas de um projeto e forneça recomendações:
          ...
          3. Riscos potenciais
          4. Próximos passos sugeridos`, 'Você é um gerente de projeto experiente especializado em metodologias ágeis.');
        ...
        writeFileSync(join(logsFolder, `${updateLog.id}.yaml`), stringifyYaml(updateLog, { indent: 2 }), 'utf-8');
    }
    catch (error) {
...
function collectAllTasks(pagiaFolder) {
    const tasks = [];
...
function analyzeTasksSync(tasks) {
    const analysis = {
        pending: 0,
        inProgress: 0,
        completed: 0,
...
function applyUpdates(pagiaFolder, updates) {
    ...
    for (const plansFolder of baseFolders) {
        ...
        for (const type of planTypes) {
            ...
            for (const file of files) {
                ...
                const tryUpdateTasks = (arrPath, arr) => {
...
function findPlanFile(pagiaFolder, name) {
    const baseFolders = [join(pagiaFolder, 'conductor'), join(pagiaFolder, 'plans')];
    const planTypes = ['global', 'stages', 'prompts', 'ai'];
