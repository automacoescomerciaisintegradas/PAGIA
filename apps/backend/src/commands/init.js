...
import { ensureGlobalDirectories, ensureProjectDirectories } from '../core/paths.js';
...
async function runMainInit(options) {
    // --- 1. Visual Welcome ---
    console.clear();
    console.log(chalk.cyan(figlet.textSync('PAGIA', {
        font: 'Slant',
        horizontalLayout: 'default',
        verticalLayout: 'default',
    })));
    console.log(boxen(`${chalk.bold('Plano de Ação de Gestão e Implementação com IA')}\n` +
    ...
    try {
        ...
        const finalConfig = await configManager.initialize({
            ...
            debug: answers.debug || false,
            aiProvider: {
                type: answers.aiProvider,
                apiKey: resolveApiKey(answers) || '',
                model: getDefaultModel(answers.aiProvider) || 'gemini-2.0-flash-exp',
            },
...
// --- Helper Functions ---
async function runSetupInterview(skip) {
    if (skip) {
        return {
...
function createConductorStructure(pagiaRoot) {
    ...
    if (!existsSync(conductorPath))
        mkdirSync(conductorPath, { recursive: true });
    dirs.forEach(dir => {
...
function createInitialGlobalPlan(globalDir, answers) {
...
function createProjectReadme(pagiaRoot, answers) {
 ...
 */
function createProjectInstructions(pagiaRoot, answers) {
 ...
 */
function createProjectSettings(pagiaRoot, answers) {
    const settings = {
        project: {
            name: answers.projectName,
...
function createModulesConfig(selected) {
    const all = [
...
async function installExtraAgents(agentsFolder) {
...
function showFinalSummary(config) {
    logger.newLine();
    console.log(boxen(`${chalk.green.bold('✅ PAGIA Inicializado com Sucesso!')}\n\n` +
