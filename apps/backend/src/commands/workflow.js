...
import { validateWorkflow } from '../agents/workflow-dag.js';
import { eventBus, PAGIAEvents } from '../core/event-bus.js';
import { START_NODE_ID, END_NODE_ID } from '../agents/workflow-types.js';
...
workflowCommand
    ...
    const files = readdirSync(workflowsDir).filter(f => f.endsWith('.yaml') || f.endsWith('.yml') || f.endsWith('.json'));
    if (files.length === 0) {
        logger.info('Nenhum workflow encontrado.');
        return;
    }
    // Header
    console.log(chalk.bold('  SERVER'.padEnd(20)) +
...
workflowCommand
    ...
    try {
        ...
        if (options.watch || options.verbose) {
            unsubscribers.push(eventBus.on(PAGIAEvents.WORKFLOW_NODE_STARTED, (event) => {
                console.log(chalk.cyan(`  ▶ Iniciando: ${event.nodeId}`));
            }));
            unsubscribers.push(eventBus.on(PAGIAEvents.WORKFLOW_NODE_COMPLETED, (event) => {
                const duration = event.result?.durationMs || 0;
                console.log(chalk.green(`  ✓ Completo: ${event.nodeId} (${duration}ms)`));
            }));
            unsubscribers.push(eventBus.on(PAGIAEvents.WORKFLOW_NODE_FAILED, (event) => {
                console.log(chalk.red(`  ✗ Falhou: ${event.nodeId} - ${event.error?.message}`));
            }));
            unsubscribers.push(eventBus.on(PAGIAEvents.WORKFLOW_NODE_RETRY, (event) => {
...
function findWorkflowFile(dir, name) {
    if (!existsSync(dir))
        return null;
...
function buildAsciiDAG(workflow) {
    const lines = [];
...
function calculateLevels(workflow) {
