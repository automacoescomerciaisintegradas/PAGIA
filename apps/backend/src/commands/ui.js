...
async function runInteractiveUI() {
...
function showHeader() {
    console.log(boxen(chalk.cyan.bold('PAGIA Configuration UI') + '\n' +
...
async function viewConfiguration(router) {
    ...
    for (const rt of ROUTER_TYPES) {
        const route = config.router[rt.value];
...
async function switchModels(router) {
    const config = router.getConfig();
    if (config.providers.length === 0) {
    ...
    await router.setRouter(routerType, provider, model);
    console.log(chalk.green(`\n✅ ${routerType} configurado para ${provider}/${model}\n`));
    await pressEnterToContinue();
}
async function addModel(router) {
    const config = router.getConfig();
    if (config.providers.length === 0) {
        console.log(chalk.yellow('\n⚠️  Nenhum provedor configurado. Adicione um provedor primeiro.\n'));
        await pressEnterToContinue();
...
async function addProvider(router, credentials) {
    ...
    let apiKey = '';
    const storedKey = await credentials.getApiKey(providerName);
    if (storedKey) {
...
async function manageCredentials(credentials) {
    const options = [
...
async function exportPreset(router) {
...
async function importPreset(router) {
...
async function showActivation(router) {
...
async function manageSettings(router, globalConfig) {
    const config = router.getConfig();
