...
export const installCommand = new Command('install')
    ...
    // Validate modules
    const invalidModules = modulesToInstall.filter((m) => !AVAILABLE_MODULES.find((am) => am.code === m));
...
async function installModule(pagiaFolder, moduleCode, verbose) {
    ...
    writeFileSync(join(moduleDir, 'config.yaml'), `# ${moduleCode} Module Configuration\n` +
        `code: ${moduleCode}\n` +
        `version: "1.0.0"\n` +
        `enabled: true\n` +
        `settings: {}\n`, 'utf-8');
...
async function createDefaultAgent(moduleDir, moduleCode) {
...
function getAgentNameForModule(moduleCode) {
    const names = {
...
function getModuleDescription(moduleCode) {
    const descriptions = {
