...
agentCommand
    ...
    if (configManager.isInitialized()) {
        ...
        for (const module of config.modules.filter((m) => m.enabled)) {
            if (options.module && options.module !== module.code)
                continue;
...
agentCommand
    ...
    try {
        ...
        if (options.ai) {
            ...
            const response = await aiService.generate(`Crie instruções detalhadas para um agente de IA com as seguintes características:
          - Nome: ${agent.name}
          - Papel: ${agent.role}
          ...
          Inclua diretrizes sobre como o agente deve se comportar e interagir.`, 'Você é um especialista em design de agentes de IA.');
...
`;
            ...
            console.log(chalk.gray(`  ${'─'.repeat(50)}`));
            console.log(chalk.gray(`  ${await getCurrentDir()} | ${chalk.cyan(providerInfo)} | ${chalk.green(response.tokensUsed || 0)} tokens`));
            console.log('');
...
agentCommand
    ...
    logger.newLine();
    logger.box(`${chalk.bold('Agentes BMAD instalados!')}
...
${chalk.cyan('pagia agent run qa')} - Testes e qualidade`, { title: '✅ Sucesso', borderColor: 'green' });
});
// Helper functions
function listAgentsInFolder(folder) {
    if (!existsSync(folder))
        return [];
...
function displayAgent(agent, verbose) {
    if (agent.file) {
    ...
    if (verbose) {
        ...
        const moduleName = agent.module || 'unknown';
...
function findAgentFile(pagiaFolder, name, config) {
    const sanitizedName = sanitizeFilename(name);
    const possiblePaths = [];
    // PRIMEIRO: Buscar nos agentes EMBUTIDOS do pacote
    const bundledFolder = getBundledAgentsFolder();
    if (bundledFolder) {
        possiblePaths.push(join(bundledFolder, `${sanitizedName}.md`), join(bundledFolder, `${name}.md`));
    }
    // DEPOIS: Buscar nos agentes locais do projeto
    if (pagiaFolder) {
        possiblePaths.push(join(pagiaFolder, 'core', 'agents', `${sanitizedName}.md`), join(pagiaFolder, 'core', 'agents', `${name}.md`));
        if (config?.modules) {
            for (const module of config.modules.filter((m) => m.enabled)) {
                possiblePaths.push(join(pagiaFolder, 'modules', module.code, 'agents', `${sanitizedName}.md`), join(pagiaFolder, 'modules', module.code, 'agents', `${name}.md`));
            }
        }
    }
 ...
 */
function getBundledAgentsFolder() {
...
function generateDefaultInstructions(agent) {
...
function getDefaultAIProvider() {
    ...
    throw new Error('Nenhuma API key de IA configurada. Configure uma das seguintes variáveis de ambiente:\n' +
        '  - GROQ_API_KEY\n' +
        '  - GEMINI_API_KEY\n' +
        '  - OPENAI_API_KEY\n' +
        '  - ANTHROPIC_API_KEY\n' +
