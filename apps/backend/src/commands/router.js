...
routerCommand
    ...
    for (const rt of ROUTER_TYPES) {
        const route = routerConfig[rt.value];
...
routerCommand
    ...
    .action(async (type, provider, model) => {
    const router = getRouterManager();
    await router.initialize();
    const config = router.getConfig();
    try {
        ...
        // Set the router
        await router.setRouter(type, provider, model);
...
providerCommand
    ...
    try {
        ...
        if (!apiKey) {
            const storedKey = await credentials.getApiKey(name);
            if (storedKey) {
        ...
        if (options?.models) {
            ...
            models = inputModels.split(',').map((m) => m.trim());
        }
        const provider = {
            name: name,
            api_base_url: apiUrl,
