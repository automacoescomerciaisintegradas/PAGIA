...
export const statusCommand = new Command('status')
    ...
    if (!configManager.isInitialized()) {
        logger.box(`${chalk.yellow('PAGIA não está inicializado neste projeto.')}\n\n` +
            `Execute ${chalk.cyan('pagia init')} para começar.`, { title: '⚠️ Status', borderColor: 'yellow' });
        return;
    }
    ...
    // Quick actions
    logger.newLine();
    logger.box(`${chalk.bold('Ações Rápidas')}\n\n` +
        `${chalk.cyan('pagia plan create')} - Criar novo plano\n` +
        `${chalk.cyan('pagia update todos')} - Atualizar tarefas\n` +
        `${chalk.cyan('pagia agent list')} - Listar agentes`, { title: '💡 Dicas', borderColor: 'cyan' });
});
