...
function handleCommand(input: string): boolean {
    ...
    switch (cmd.toLowerCase()) {
        ...
        case '/s':
            ...
            try {
                provider = new MultiProvider(args[0] as any);
        ...
        case '/config':
            ...
            console.log(`  Fallback:          ${cfg.get<string>('ai_provider.fallback') || 'nenhum'}`);
            console.log(`  Profile ativo:     ${chalk.yellow(cfg.get<string>('profile') || 'default')}`);
...
async function run(providerName?: string): Promise<void> {
    startTime = Date.now();
    ...
    try {
        ...
        provider = new MultiProvider((providerName || defaultProvider) as any);
    } catch (e: any) {
...
export async function startDefaultREPL(): Promise<void> {
    await run();
}
