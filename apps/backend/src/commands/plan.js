    ...
    try {
        ...
        if (options.open) {
            try {
                const editor = process.env.EDITOR || 'code';
                const { spawn } = await import('child_process');
                spawn(editor, [result.path || ''], { stdio: 'ignore', detached: true }).unref();
                logger.info('Abrindo arquivo no editor...');
            }
            catch (err) {
                logger.warn('Não foi possível abrir o editor automaticamente.');
...
planCommand
    ...
    for (const type of types) {
        ...
        if (existsSync(planFile)) {
            ...
            logger.box(stringifyYaml(content, { indent: 2 }), { title: `📋 ${content.name || name}`, borderColor: 'cyan' });
            return;
        }
    }
...
// Helper functions
async function createGlobalPlan(name, useAI, config) {
    const plan = {
        id: generateId(),
        name,
        description: '',
    ...
    if (useAI) {
        ...
        const response = await aiService.generate(`Crie um plano de ação global de alto nível para o projeto "${name}". 
      Inclua 3-5 objetivos principais, 4-6 etapas de desenvolvimento e 3-4 marcos importantes.
      Responda em JSON com a estrutura: { objectives: [...], stages: [...], milestones: [...] }`, 'Você é um especialista em gestão de projetos e planejamento estratégico.');
        try {
...
async function createStagePlan(name, useAI, config) {
    return {
        id: generateId(),
        name,
        description: '',
...
async function createPromptPlan(name, config) {
    ...
    const aiService = createAIService(config.aiProvider);
    const response = await aiService.generate(`Analise este prompt e gere um plano de ação: "${prompt}"
...
async function createAIPlan(name, config) {
    const aiService = createAIService(config.aiProvider);
    const response = await aiService.generate(`Analise o contexto do projeto "${name}" e gere um plano de ação autônomo.
...
export function listTemplates(pagiaFolder, type) {
    ...
    for (const t of types) {
        ...
        for (const f of files) {
            if (f.startsWith('example-'))
                continue; // skip examples by default
            results.push({ type: t, file: f });
        }
    }
    return results;
}
export function listTemplatesDetailed(pagiaFolder, type) {
...
export async function installTemplate(pagiaFolder, templateName, targetDir, opts) {
    ...
    const content = parseYaml(readFileSync(found.fullPath, 'utf-8'));
...
export async function installTemplateInteractive(pagiaFolder, templateName, targetDir, opts) {
    ...
    if (existsSync(targetPath) && !opts?.force) {
        ...
        if (!answer.ok)
            return { canceled: true, path: targetPath };
...
export async function generatePlanFile(pagiaFolder, type, name, template) {
    ...
    if (template) {
        ...
        const content = parseYaml(readFileSync(templatePath, 'utf-8'));
        content.name = name;
        writeFileSync(targetPath, stringifyYaml(content, { indent: 2 }), 'utf-8');
        return targetPath;
    }
...
export function humanizeTemplateName(template) {
    if (!template)
        return 'New Plan';
 ...
 */
export async function installTemplateCLI(pagiaDir, name, targetDir, opts) {
