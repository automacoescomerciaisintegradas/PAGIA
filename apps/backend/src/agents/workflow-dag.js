 ...
 */
import { START_NODE_ID, END_NODE_ID, DEFAULT_WORKFLOW_CONFIG, WORKFLOW_LIMITS, } from './workflow-types.js';
// ============================================================================
// DAG Builder
// ============================================================================
/**
 ...
 */
export function getReadyNodes(workflow, completedNodes) {
    ...
    for (const node of workflow.nodes) {
        ...
        const allDependenciesMet = dependencies.every(dep => dep === START_NODE_ID || completedNodes.has(dep));
 ...
 */
function analyzeConnectivity(workflow) {
    const adjacencyList = buildAdjacencyList(workflow);
    ...
    const queueStart = [START_NODE_ID];
    while (queueStart.length > 0) {
        const node = queueStart.shift();
        if (reachableFromStart.has(node))
            continue;
    ...
    const reachesToEnd = new Set();
    const queueEnd = [END_NODE_ID];
    while (queueEnd.length > 0) {
        const node = queueEnd.shift();
        if (reachesToEnd.has(node))
            continue;
 ...
 */
function buildAdjacencyList(workflow) {
 ...
 */
function buildReverseAdjacencyList(workflow) {
