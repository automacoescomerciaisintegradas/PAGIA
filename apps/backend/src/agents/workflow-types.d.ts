 ...
 * Função para mapear contexto do workflow para input do agente
 */
export type InputMapperFunction = (context: WorkflowExecutionContext, node: WorkflowNode) => AgentInput;
/**
 * Função para mapear output do agente para atualizações no contexto
 */
export type OutputMapperFunction = (output: AgentOutput, context: WorkflowExecutionContext) => Partial<Record<string, unknown>>;
/**
 * Condição para traversar uma aresta
 */
export type EdgeConditionFunction = (context: WorkflowExecutionContext) => boolean;
/**
 * Função agregadora de múltiplos outputs
 */
export type AggregatorFunction = (outputs: AgentOutput[], context: WorkflowExecutionContext) => AgentOutput;
/**
 * Tipos de eventos de workflow
 */
export type WorkflowEventType = 'workflow:started' | 'workflow:completed' | 'workflow:failed' | 'workflow:timeout' | 'workflow:cancelled' | 'workflow:node:queued' | 'workflow:node:started' | 'workflow:node:completed' | 'workflow:node:failed' | 'workflow:node:retry' | 'workflow:node:skipped' | 'workflow:branch:merged';
/**
 ...
 */
export type WorkflowEvent = WorkflowStartedEvent | WorkflowCompletedEvent | WorkflowFailedEvent | WorkflowNodeStartedEvent | WorkflowNodeCompletedEvent | WorkflowNodeFailedEvent | WorkflowNodeRetryEvent | WorkflowBranchMergedEvent;
/**
...
export declare const END_NODE_ID = "__end__";
