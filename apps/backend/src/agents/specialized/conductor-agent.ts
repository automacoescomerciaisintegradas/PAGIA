...
import type { WorkflowDefinition, WorkflowNode, WorkflowEdge } from '../workflow-types.js';
...
Responda considerando o contexto do projeto e as melhores práticas de desenvolvimento.
     ...
     */
    private async executeParallel(input: AgentInput): Promise<string> {
        // Carregar contexto
        this.loadContext();
...
Estas tarefas serão executadas simultaneamente quando você usar \`/parallel\`.`;
        ...
        const pendingBlock = parallelBlocks.find(block =>
            block.tasks.some(t => t.status === 'pending')
        );
        ...
        for (const [nodeId, nodeResult] of result.nodeResults) {
            ...
            if (task) {
                ...
                finalPlan = finalPlan.replace(
                    `- [~] ${task.name}`,
                    `- [${marker}] ${task.name} <!-- parallel: ${result.metrics.totalDurationMs}ms -->`
                );
            }
        }
...
3. Use \`/parallel\` para o próximo bloco, ou \`/implement\` para tarefas sequenciais`;
     ...
     */
    private parseParallelBlocks(plan: string): Array<{
        startIndex: number;
        endIndex: number;
        tasks: Array<{ id: string; name: string; status: 'pending' | 'in-progress' | 'completed' }>;
    }> {
        const blocks: Array<{
            startIndex: number;
            endIndex: number;
     ...
     */
    private parseTasksFromBlock(content: string): Array<{
        id: string;
        name: string;
        status: 'pending' | 'in-progress' | 'completed';
    }> {
        const tasks: Array<{ id: string; name: string; status: 'pending' | 'in-progress' | 'completed' }> = [];
