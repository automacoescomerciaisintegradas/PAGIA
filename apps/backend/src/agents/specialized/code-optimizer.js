...
export class CodeOptimizerAgent extends BaseAgent {
    ...
    role = 'Especialista em otimização e refatoração de código';
    description = 'Analisa e otimiza código para melhor performance, legibilidade e segurança';
    module = 'core';
    capabilities = [
        'análise de código',
...
- Forneça métricas quando aplicável
  `;
    ...
    parseOptions(input) {
        ...
        return {
            target: context.target || this.defaultOptions.target,
     ...
     */
    buildAnalyzePrompt(code, options) {
        return `
...
7. **Recomendações**: Priorize melhorias por impacto
     ...
     */
    buildOptimizePrompt(code, options) {
        return `
...
Forneça código corrigido quando aplicável.
     ...
     */
    buildComplexityPrompt(code, options) {
        return `
...
${options.preserveComments ? 'Preserve comentários.' : ''}
     ...
     */
    async analyzeCode(code, language = 'auto', target = 'all') {
        return this.execute({
            prompt: `/analyze ${code}`,
     ...
     */
    async refactorCode(code, language = 'auto') {
        return this.execute({
     ...
     */
    async securityAudit(code, language = 'auto') {
        return this.execute({
