...
export class TesterAgent extends BaseAgent {
    ...
    role = 'Especialista em TDD e Quality Assurance';
    description = 'Gera testes automatizados, analisa cobertura e guia desenvolvimento TDD';
    module = 'core';
    capabilities = [
...
- Documente cenários não óbvios
  `;
    ...
    parseOptions(input) {
        ...
        return {
            framework: context.framework || this.defaultOptions.framework,
            testType: context.testType || this.defaultOptions.testType,
            language: context.language || this.defaultOptions.language,
     ...
     */
    buildGeneratePrompt(code, options) {
...
Forneça testes completos e executáveis.
     ...
     */
    buildTDDPrompt(requirement, options) {
        return `
...
Comece pela Fase 1 (RED).
     ...
     */
    buildCoveragePrompt(code, options) {
        return `
...
Sugira testes para atingir 80%+ de cobertura.
     ...
     */
    buildMockPrompt(dependencies, options) {
        return `
...
\`\`\`
     ...
     */
    buildFixPrompt(failingTest, options) {
        return `
...
indique o que precisa mudar na implementação.
     ...
     */
    buildEdgeCasesPrompt(code, options) {
        return `
...
Priorize por risco de bug real.
     ...
     * Prompt geral
     */
    buildGeneralPrompt(userPrompt, options) {
        return `
...
Forneça código de teste completo e executável.
     ...
     */
    async generateTests(code, language = 'typescript', framework = 'auto') {
        return this.execute({
     ...
     * Inicia ciclo TDD
     */
    async startTDD(requirement, language = 'typescript') {
        return this.execute({
     ...
     * Analisa cobertura
     */
    async analyzeCoverage(code, language = 'typescript') {
        return this.execute({
     ...
     * Gera mocks
     */
    async generateMocks(dependencies, language = 'typescript') {
        return this.execute({
     ...
     * Corrige teste
     */
    async fixTest(failingTest, framework = 'auto') {
        return this.execute({
     ...
     * Sugere edge cases
     */
    async suggestEdgeCases(code, language = 'typescript') {
        return this.execute({
