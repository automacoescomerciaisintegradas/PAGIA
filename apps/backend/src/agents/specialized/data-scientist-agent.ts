...
Siga o processo de análise de dados: entender requisitos, planejar abordagem, executar análise, interpretar resultados e fornecer insights acionáveis.`;
    ...
    private generateSQLQuery(requirement: string): string {
...
LIMIT 100`;
    ...
    private getTableName(requirement: string): string {
        // Extrai o nome da tabela do requisito ou retorna um padrão
        const lowerReq = requirement.toLowerCase();
        if (lowerReq.includes('usuário') || lowerReq.includes('user')) return 'projeto.dataset.usuarios';
        if (lowerReq.includes('pedido') || lowerReq.includes('order')) return 'projeto.dataset.pedidos';
        if (lowerReq.includes('produto') || lowerReq.includes('product')) return 'projeto.dataset.produtos';
        return 'projeto.dataset.tabela_padrao';
    }

    private generateSampleQuery(requirement: string): string {
...
ORDER BY media_valor DESC`;
    ...
    private generateOptimizedQuery(requirement: string): string {
