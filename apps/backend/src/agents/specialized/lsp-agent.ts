...
`;
        ...
        try {
            ...
            if (!parsedCommand) {
                return this.createOutput(
                    '❌ Comando não reconhecido. Use um dos comandos disponíveis no menu.',
                    undefined,
                    startTime
                );
            }
    ...
    private parseCommand(prompt: string): { command: string; args: any } | null {
    ...
    private formatResult(command: string, result: any): string {
        switch (command) {
            case 'goto-definition':
    ...
    private async handleHoverInfo(args: any) {
        // Placeholder para implementação real
        return {
            symbol: args.symbol || 'symbol_not_provided',
