...
// Tipos inferidos do AgentKit
type AgentKitNetwork = ReturnType<typeof createNetwork>;
type AgentKitAgent = ReturnType<typeof createAgent>;
...
export class PAGIANetwork {
    private static instance: PAGIANetwork;
    private networks: Map<string, AgentKitNetwork> = new Map();
    ...
    private constructor() {
        this.defaultConfig = {
            name: 'pagia-default',
            defaultModel: {
                provider: (process.env.AI_PROVIDER as 'gemini' | 'openai' | 'anthropic') || 'gemini',
     ...
     */
    private createModel(_provider: string, model: string) {
     ...
     */
    private createPlannerAgent(model: ReturnType<typeof gemini>): AgentKitAgent {
        return createAgent({
            name: 'Planner',
...
4. Use a ferramenta 'save_plan' para armazenar o plano`,
     ...
     */
    private createExecutorAgent(model: ReturnType<typeof gemini>): AgentKitAgent {
        return createAgent({
            name: 'Executor',
...
3. Use 'complete_step' quando terminar`;
            ...
            tools: [
                createTool({
                    ...
                    handler: async ({ result, notes }, { network }) => {
                        ...
                        const results = (network?.state.kv.get('results') || {}) as Record<string, string>;
     ...
     * Cria agente revisor
     */
    private createReviewerAgent(model: ReturnType<typeof gemini>): AgentKitAgent {
        return createAgent({
            name: 'Reviewer',
...
4. Use 'approve_results' ou 'request_revision'`;
    ...
    private createSupervisorAgent(
        model: ReturnType<typeof gemini>,
...
- Use 'done' quando tudo estiver concluído`,
    ...
    createDefaultNetwork(config?: Partial<NetworkConfig>): AgentKitNetwork {
        ...
        const model = this.createModel(
            finalConfig.defaultModel!.provider,
     ...
     */
    async createCustomNetwork(
        name: string,
        agentIds: string[],
        config?: Partial<NetworkConfig>
    ): Promise<AgentKitNetwork> {
...
${pagiaAgent.instructions || 'Siga as melhores práticas da sua especialidade.'}`,
        ...
        const network = createNetwork({
            ...
            router: ({ network: net }) => {
                ...
                const currentAgentIndex = net?.state.kv.get('currentAgentIndex') as number || 0;
     ...
     * Executa uma rede
     */
    async runNetwork(networkName: string, input: string): Promise<NetworkRunResult> {
        ...
        try {
            ...
            return {
                success: true,
                result: finalSummary as string || 'Execução concluída',
                state,
            };
        } catch (error) {
     ...
     */
    getStats(): { total: number; networks: string[] } {
        return {
