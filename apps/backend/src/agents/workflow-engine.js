...
import { START_NODE_ID, END_NODE_ID, DEFAULT_RETRY_POLICY, } from './workflow-types.js';
// ============================================================================
// Workflow Engine
// ============================================================================
/**
...
export class WorkflowEngine {
    ...
    async execute(workflow, input) {
        ...
        const timeoutPromise = config.timeout
            ? new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new Error('Workflow timeout')), config.timeout);
            })
            : null;
        try {
            // Executar workflow
            const executionPromise = this.executeNodes(workflow, context, semaphore, config);
            ...
            const allCompleted = workflow.nodes.every(n => context.completedNodes.has(n.id));
            const completedAt = new Date();
            const metrics = this.calculateMetrics(context, startedAt, completedAt);
            // Agregar resultados
            const aggregatedOutput = this.aggregateResults(workflow, context, config.aggregator);
            const result = {
                executionId,
                workflowId: workflow.id,
            ...
            await this.emitWorkflowFailed(result, error);
            return result;
        }
        finally {
            if (timeoutId) {
     ...
     */
    async executeNodes(workflow, context, semaphore, config) {
        const nodeMap = new Map(workflow.nodes.map(n => [n.id, n]));
        const completedSet = new Set([START_NODE_ID]);
        ...
        while (context.pendingNodes.size > 0 || context.runningNodes.size > 0) {
            ...
            const nodesToStart = readyNodes.filter(id => !context.runningNodes.has(id) && context.pendingNodes.has(id));
            if (nodesToStart.length === 0 && context.runningNodes.size === 0) {
                ...
                if (context.pendingNodes.size > 0) {
                    throw new Error(`Deadlock: nodos pendentes sem dependências satisfeitas: ${Array.from(context.pendingNodes).join(', ')}`);
                }
                break;
            }
            ...
            const nodePromises = nodesToStart.map(async (nodeId) => {
                ...
                try {
                    const result = await this.executeNode(node, context, semaphore, config);
     ...
     */
    async executeNode(node, context, semaphore, config) {
        const startedAt = new Date();
        let attempts = 0;
        ...
        try {
            ...
            const retryResult = await withRetryPolicy(async () => agent.safeExecute(nodeInput), retryPolicy, {
     ...
     */
    defaultInputMapper(context, node) {
        // Coletar outputs de nodos dependentes
        const dependentOutputs = [];
     ...
     */
    aggregateResults(workflow, context, customAggregator) {
        ...
        const content = successfulOutputs
            ...
            .join('\n\n---\n\n');
        const totalTokens = successfulOutputs.reduce((sum, o) => sum + (o.metadata.tokensUsed || 0), 0);
        return {
            content,
            metadata: {
     ...
     */
    calculateMetrics(context, startedAt, completedAt) {
     ...
     */
    async checkBranchMerge(workflow, context, completedNodeId) {
        ...
        for (const edge of dependentEdges) {
            ...
            const allPredecessorsComplete = predecessorEdges.every(e => e.from === START_NODE_ID || context.completedNodes.has(e.from));
    ...
    // ========================================================================
    async emitWorkflowStarted(workflow, input, executionId) {
    ...
    async emitWorkflowFailed(result, error) {
    ...
    async emitNodeStarted(context, node) {
    ...
    async emitNodeCompleted(context, node, result) {
    ...
    async emitNodeRetry(context, node, error, attempt, delayMs, maxAttempts) {
